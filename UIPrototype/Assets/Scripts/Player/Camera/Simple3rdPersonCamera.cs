...
public class Simple3rdPersonCamera : MonoBehaviour
{
    ...
    private void GetInputs()
    {
        ...
        if (playerToFollow.gamepadInput)
        {
            if (playerToFollow.myGamepad.IsMovingRightStick)
            {
                currentX += playerToFollow.myGamepad.GetStick_R().X;

                if (invertYAxis) { currentY += playerToFollow.myGamepad.GetStick_R().Y; }
                else { currentY -= playerToFollow.myGamepad.GetStick_R().Y; }
            }
        }
    }
}
